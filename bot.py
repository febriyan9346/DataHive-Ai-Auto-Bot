...
"""
    ...
    def load_proxies(self, use_proxy_choice: int):
        ...
        try:
            if use_proxy_choice == 1:
                self.log("Downloading proxies from Proxyscrape...", "INFO")
    ...
    def show_menu(self):
        ...
        print(f"{Fore.GREEN}1. Run with Free Proxyscrape Proxy")
        print(f"2. Run with Private Proxy")
        print(f"3. Run without proxy{Style.RESET_ALL}")
        print(f"{Fore.CYAN}============================================================{Style.RESET_ALL}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'user' in data:
                    ...
                    self.log(f"Total Points: {total_points:.2f} | Today: +{gained_24h:.2f}", "SUCCESS")
    ...
    def ping_uptime(self, token, device_info, account_num, email, proxy=None):
        try:
            ...
            if response.status_code == 200:
                ...
                self.log(f"PING Success | Uptime: {uptime:.4f}", "SUCCESS")
                return True
            else:
    ...
    def check_worker_ip(self, token, device_info, account_num, email, proxy=None):
        try:
            ...
            if response.status_code == 200:
                ...
                self.log(f"IP: {ip} | Country: {country}", "SUCCESS")
                return True
            else:
    ...
    def run(self):
        ...
        if use_proxy_choice == 1:
            self.log("Running with Free Proxyscrape Proxy", "INFO")
        elif use_proxy_choice == 2:
            self.log("Running with Private Proxy", "INFO")
        else:
        ...
        try:
            ...
            self.log(
                f"Statistics:\n"
                f"   • Total Cycles: {cycle - 1}\n"
                f"   • Total Accounts: {len(active_workers)}\n"
                f"   • Total Points Gained: +{total_gained_final:.2f} PTS",
                "INFO"
            )
